...
function Calendar({
  ...
  const handleSelect = (day: Date | undefined, ...args: any[]) => {
      if (onSelect) {
          const newDate = day ? new Date(day) : undefined;
          if (newDate && selected instanceof Date) {
              newDate.setHours(selected.getHours());
              newDate.setMinutes(selected.getMinutes());
          }
          (onSelect as any)(newDate, ...args);
      }
  };
  
  return (
    <div className={cn("inline-block", time && "border rounded-md")}>
      <DayPicker
        ...
        components={{
          ...
          Dropdown: ({...props}: DropdownProps) => {
            ...
            return (
              ...
              >
                <SelectTrigger>{props.value?.toString()}</SelectTrigger>
                <SelectContent>
                   <ScrollArea className="h-48">
                  {options?.map(option => (
      ...
      />
      {time && selected && <TimePicker date={selected as Date} setDate={onSelect as any} />}
    </div>
  )
}
